...
const Tracking = () => {
  ...
  const [isPopupVisibleNotification, setIsPopupVisibleNotification] =
    useState(false);
  const [isPopupVisibleOrderDetail, setIsPopupVisibleOrderDetail] =
    useState(false);
  ...
  return (
    <div className="w-full mx-auto my-4 mb-4">
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th className="px-2 py-3 text-center text-gray-600">
                Mã hóa đơn
              </th>
              ...
              <th className="flex items-center justify-center px-2 py-3 text-center text-gray-600">
                Hành động
              </th>
            </tr>
          </thead>
              ...
              orderList.map((order, index) => (
                <tr key={order.id} className="border-t">
                  ...
                  <td className="px-2 py-2 text-center text-red-500">
                    {formatCurrencyVND(order.grand_total)}
                  </td>
                    ...
                    {order.order_status === "canceled" ? (
                      ...
                      <>
                        <button
                          ...
                          className="px-2 py-2 rounded-lg border border-[#BC9A6C] bg-[#BC9A6C] text-white"
                        >
                          Chi tiết
                        </button>
                        {order.order_status !== "completed" &&
                          ...
                          (order.payment_method === "vnpay" ? (
                            <button
                              onClick={() => handleCancelOrderVNpay(order)}
                              className="px-2 py-2 rounded-lg border border-[#ff0000] bg-[#ff0000] text-white"
                            >
                              Hủy đơn hàng
                            </button>
                          ) : (
                            <button
                              onClick={() => handleCancelOrderCOD(order)}
                              className="px-2 py-2 rounded-lg border border-[#ff0000] bg-[#ff0000] text-white"
                            >
                              Hủy đơn hàng
                            </button>
                        ...
                        {order.order_status === "completed" && (
                          <button
                            onClick={() => handleRatingOrder(order)}
                            className="px-2 py-2 rounded-lg border border-[#FFD700] bg-[#FFD700] text-white"
                          >
                            Viết đánh giá
                          </button>
                        )}
      ...
      {isPopupVisibleRating && (
        <Rating
          onClose={() => setIsPopupVisibleRating(false)}
          order={selectedOrder}
        />
      )}
      {isPopupVisibleNotification && (
        <Notification
          onClose={() => setIsPopupVisibleNotification(false)}
          order={selectedOrder}
        />
      )}
      {isPopupVisibleOrderDetail && (
        <OrderDetail
          onClose={() => setIsPopupVisibleOrderDetail(false)}
          order={selectedOrder}
        />
      )}
    </div>
